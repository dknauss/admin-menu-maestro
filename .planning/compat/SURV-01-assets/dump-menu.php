<?php
/**
 * Natural-state $menu / $submenu dump for the WooCommerce compat survey (SURV-01).
 *
 ...
 * is_admin() is false, so WooCommerce's classic WC_Admin_Menus class (which
 * registers the top-level `woocommerce` item, `separator-woocommerce`, the
 * Attributes submenu, and the Reports/Settings/Status/Add-ons submenus) never
 * instantiates, and the dump is silently INCOMPLETE (the top-level WooCommerce
 * menu is missing). --exec runs before WP bootstraps, so the constant is set in
 * time for the admin-only init paths to fire.
 *
