<?php
/**
 * Natural-state $menu / $submenu dump for the Elementor compat survey (SURV-04).
 *
 ...
 * is_admin() is false, so Elementor's menu registration paths that gate on
 * is_admin() / the admin-context init paths will not fire, producing a silently
 * incomplete dump. --exec runs before WP bootstraps, so the constant is set in
 * time for the admin-only init paths to fire. (Confirmed at runtime: with
 * WP_ADMIN=true, all three Elementor top-levels and every submenu appear; without
 * it, Elementor menus are absent or heavily reduced.)
 *
