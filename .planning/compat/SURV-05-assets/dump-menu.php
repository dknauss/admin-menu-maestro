<?php
/**
 * Natural-state $menu / $submenu dump for the WPForms Lite compat survey (SURV-05).
 *
 ...
 * is_admin() is false and WPForms never hooks admin_menu, so the top-level
 * `wpforms-overview` item and all submenus are silently absent from the dump.
 * --exec runs before WP bootstraps, so the constant is set in time for the
 * admin-only init paths to fire.
 *
