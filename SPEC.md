...
- **Keyboard reordering (Alt+Arrow).** After selecting an item, press `Alt+ArrowUp` or `Alt+ArrowDown` to move it within its own list (top-level items move among top-level; submenu items move among siblings). Focus is restored to the moved row's anchor after each re-append so that chained presses work without re-selection. Successful moves are announced politely via `wp.a11y.speak()` with position ("Posts moved down, position 3 of 8"); reaching a boundary is announced assertively ("Posts is already last"). The shortcut is also exposed on the selected row via `aria-keyshortcuts="Alt+ArrowUp Alt+ArrowDown"` for AT discovery.
- **Modified indicator (non-color-only, WCAG 1.4.1 / 1.4.11).** Every item that differs from its WordPress default shows a filled bullet glyph (•) on its row in edit mode. The glyph is amber (`#dba617`) against the dark admin-menu background (`#1d2327`), giving approximately 5.5:1 contrast (above the 3:1 minimum for graphical objects under WCAG 1.4.11). A `.screen-reader-text` sibling "(modified)" exposes the semantic state to assistive technology; color is never the sole signal (WCAG 1.4.1). The indicator refreshes live on every rename, icon change, visibility toggle, and per-item reset, and on page load for pre-existing overrides.
- **Discoverable per-item reset.** The **Reset this item** button in the controls panel is visible whenever an item is selected (not hidden behind prior knowledge). When the selected item is modified, the button is visually emphasised (amber styling) and has a `:focus-visible` outline for keyboard users. Tab to the button and press `Enter` or `Space` to reset; the indicator clears and the item's delta is removed from the saved configuration.
